/* Adelinemagica - panel privado (lista de consultas + carta natal + informe) */
(function () {
  "use strict";
  ...
  function loadCalendar() {
    api("/api/admin/calendar-events?days=180")
      .then(function (events) {
        ...
        if (!adminCalendar) {
          adminCalendar = new window.FullCalendar.Calendar(el, {
            ...
            eventClick: function (info) {
              ...
              if (p.source !== "direct") return;
              var id = String(info.event.id || "");
              if (id.indexOf("direct-") !== 0) return;
              var intakeId = id.replace("direct-", "");
              var answer = window.prompt(
                "Google Calendar: escribe tentative, confirmed o cancelled",
                (p.google_event_status || "tentative")
              );
              if (!answer) return;
              var status = String(answer || "").trim().toLowerCase();
              if (["tentative", "confirmed", "cancelled"].indexOf(status) === -1) {
                window.alert("Estado invalido.");
                return;
              }
              apiSend("/api/admin/direct-intakes/" + encodeURIComponent(intakeId) + "/google-status?status=" + encodeURIComponent(status), "POST")
                .then(function () { loadCalendar(); })
                .catch(function (err) {
                  if (err.message === "401") { logout(); return; }
                  window.alert(err.message || "No se pudo sincronizar Google Calendar.");
                });
            },
            eventDidMount: function (info) {
              var p = info.event.extendedProps || {};
              var source = p.source || "";
              if (source === "direct") info.el.style.borderColor = "#60c689";
              if (source === "booking") info.el.style.borderColor = "#a88de0";
              info.el.title = [
                info.event.title,
                p.email ? ("Email: " + p.email) : "",
                p.reference ? ("Ref: " + p.reference) : "",
                p.service ? ("Servicio: " + p.service) : "",
                p.google_event_status ? ("Google: " + p.google_event_status) : "",
                p.google_last_error ? ("Google error: " + p.google_last_error) : "",
              ].filter(Boolean).join("\n");
            }
          });
        ...
        (events || []).forEach(function (ev) { adminCalendar.addEvent(ev); });
      })
      .catch(function (err) {
        if (err.message === "401") { logout(); return; }
      });
  ...
  function renderDetail(b) {
    ...
    var video = b.video_url
      ? '<a href="' + esc(b.video_url) + '" target="_blank" rel="noopener" class="btn btn-ghost" style="margin-top:8px">Abrir sala (solo Adelinemagica)</a>'
    ...
    var right =
      '<h3 style="color:var(--lilac);margin:0 0 8px;font-size:1rem;letter-spacing:.1em">INFORME PARA LA SESION</h3>' +
